...
class Sizer {
  ...
  static SizeResult _qr(EncodeConfig cfg) {
    ...
    if (version == null) {
      return _overflow(cfg, 'Data exceeds QR capacity at EC ${cfg.ecLevel.label}');
    }
    ...
    final budget = _logo(cfg, symbolSideMm, cfg.ecLevel.recoverableFraction,
        warnings, is2D: true);
    return SizeResult(
  ...
  static SizeResult _dataMatrix(EncodeConfig cfg) {
    ...
    final budget = _logo(cfg, symbolSideMm, size.correctionFraction, warnings,
        is2D: true);
    warnings.add(
  ...
  static LogoBudget? _logo(EncodeConfig cfg, double symbolSideMm,
      double recoverable, List<String> warnings,
      {required bool is2D}) {
